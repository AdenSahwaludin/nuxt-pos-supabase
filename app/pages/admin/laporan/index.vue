<template>
  <div class="min-h-screen bg-gradient-to-br from-gray-50 to-blue-50 p-6">
    ...
    <div class="mb-8 text-center">
      ...
      <p class="text-gray-600 mt-2 text-lg">Analisis dan monitor performa bisnis Anda</p>
    </div>

    <!-- Global Filters with Enhanced Design -->
    <div class="bg-white/90 rounded-2xl shadow-xl border border-gray-200 p-6 mb-8 backdrop-blur-sm">
      <div class="flex items-center mb-6">
        <div class="p-2 bg-blue-100 rounded-lg mr-3">
          <svg class="w-5 h-5 text-blue-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4a1 1 0 011-1h.01a1 1 0 010 2H4a1 1 0 01-1-1zm0 4a1 1 0 011-1h.01a1 1 0 010 2H4a1 1 0 01-1-1zm0 4a1 1 0 011-1h.01a1 1 0 010 2H4a1 1 0 01-1-1zm4-8a1 1 0 011-1h6a1 1 0 110 2H8a1 1 0 01-1-1zm0 4a1 1 0 011-1h6a1 1 0 110 2H8a1 1 0 01-1-1zm0 4a1 1 0 011-1h6a1 1 0 110 2H8a1 1 0 01-1-1z"></path>
          </svg>
        </div>
        <h3 class="text-xl font-semibold text-gray-900">Filter Global</h3>
        <div class="ml-auto">
          <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
            {{ appliedFilters.periode === 'today' ? 'Hari Ini' : appliedFilters.periode === 'yesterday' ? 'Kemarin' : appliedFilters.periode === 'week' ? 'Minggu Ini' : appliedFilters.periode === 'month' ? 'Bulan Ini' : 'Custom' }}
          </span>
        </div>
      </div>
    ...
    <div class="bg-white/90 rounded-2xl shadow-2xl border border-gray-200 backdrop-blur-sm overflow-hidden">
      <!-- Tab Navigation with Gradient -->
      <div class="bg-gradient-to-r from-gray-50 to-blue-50 border-b-2 border-gray-200">
      ...
      <div class="p-6">
        ...
        <div v-if="activeTab === 'dashboard'" class="space-y-6">
          <RingkasanPenjualan :filters="appliedFilters" @drilldown="handleDrilldown" />
        </div>

        <!-- Tren Penjualan -->
        <div v-else-if="activeTab === 'trend'" class="space-y-6">
          <TrenPenjualan :filters="appliedFilters" @drilldown="handleDrilldown" />
        </div>

        <!-- Penjualan per Kategori -->
        <div v-else-if="activeTab === 'category'" class="space-y-6">
          <PenjualanKategori :filters="appliedFilters" @drilldown="handleDrilldown" />
        </div>

        <!-- Penjualan per Produk -->
        <div v-else-if="activeTab === 'product'" class="space-y-6">
          <PenjualanProduk :filters="appliedFilters" @drilldown="handleDrilldown" />
        </div>
        ...
        <div v-else-if="activeTab === 'payment'" class="space-y-6">
          <MetodePembayaran :filters="appliedFilters" @drilldown="handleDrilldown" />
        </div>

        <!-- Transaksi Bermasalah -->
        <div v-else-if="activeTab === 'problem'" class="space-y-6">
          <TransaksiBermasalah :filters="appliedFilters" @drilldown="handleDrilldown" />
        </div>

        <!-- Fast vs Slow Moving -->
        <div v-else-if="activeTab === 'moving'" class="space-y-6">
          <FastSlowMoving :filters="appliedFilters" @drilldown="handleDrilldown" />
        </div>
        ...
        <div v-else-if="activeTab === 'margin'" class="space-y-6">
          <MarginAnalysis :filters="appliedFilters" @drilldown="handleDrilldown" />
        </div>

        <!-- Penjualan per Pelanggan -->
        <div v-else-if="activeTab === 'customer'" class="space-y-6">
          <PenjualanPelanggan :filters="appliedFilters" @drilldown="handleDrilldown" />
        </div>

        <!-- Performa Kasir -->
        <div v-else-if="activeTab === 'cashier'" class="space-y-6">
          <PerformaKasir :filters="appliedFilters" @drilldown="handleDrilldown" />
        </div>

        <!-- Monitoring Pajak -->
        <div v-else-if="activeTab === 'tax'" class="space-y-6">
          <MonitoringPajak :filters="appliedFilters" @drilldown="handleDrilldown" />
        </div>

        <!-- Rekonsiliasi -->
        <div v-else-if="activeTab === 'reconcile'" class="space-y-6">
          <RekonsiliasiBayar :filters="appliedFilters" @drilldown="handleDrilldown" />
        </div>
...
const updateDateFromPeriode = () => {
  ...
  switch (globalFilters.value.periode) {
    ...
    case 'month':
      ...
      globalFilters.value.dateFrom = startOfMonth.toISOString().substring(0, 10);
      globalFilters.value.dateTo = todayStr;
      break;
    case 'custom':
      // Keep current values
