<template>
  <div class="space-y-6">
    <!-- Chart Controls -->
    <div
      class="bg-gradient-to-br from-white to-slate-50 rounded-2xl shadow-lg p-6 border border-slate-200"
    >
      <div class="flex items-center justify-between mb-6">
        <div class="flex items-center">
          <div
            class="p-3 bg-gradient-to-r from-blue-500 to-blue-600 rounded-xl shadow-lg"
          >
            <svg
      ...
      <div class="flex items-center gap-6 mb-6">
        <label class="flex items-center cursor-pointer group">
          ...
          />
          <span
            class="ml-3 text-sm font-medium text-slate-700 group-hover:text-slate-900 transition-colors"
            >📊 Tampilkan Moving Average</span
          >
        </label>
      ...
      <!-- Chart Placeholder -->
      <div
        class="h-80 bg-gray-50 rounded-lg flex items-center justify-center relative"
      >
        ...
        <div v-else class="w-full h-full flex items-center justify-center">
          ...
          <div class="text-center">
            <svg
              class="w-16 h-16 mx-auto text-blue-500 mb-2"
              fill="none"
              stroke="currentColor"
              viewBox="0 0 24 24"
            >
    ...
    <!-- Summary Table -->
    <div
      class="bg-gradient-to-br from-slate-50 to-slate-100 rounded-2xl shadow-lg p-6 border border-slate-200"
    >
      <div class="flex items-center justify-between mb-6">
        <div class="flex items-center">
          <div
        ...
        <table class="min-w-full">
          <thead>
            <tr class="bg-gradient-to-r from-slate-100 to-slate-200">
              <th
                class="px-6 py-4 text-left text-xs font-bold text-slate-700 uppercase tracking-wider rounded-tl-xl"
              >
                📅 Periode
              </th>
              <th
              ...
              <th
                class="px-6 py-4 text-right text-xs font-bold text-slate-700 uppercase tracking-wider rounded-tr-xl"
              >
                ⚡ Aksi
              </th>
            </tr>
          ...
          <tbody class="bg-white">
            ...
            >
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex items-center">
                  <div
                    class="w-10 h-10 bg-gradient-to-r from-blue-100 to-blue-200 rounded-full flex items-center justify-center text-blue-700 font-bold text-sm shadow-sm mr-3"
                  >
                    {{ index + 1 }}
                  </div>
                  <div>
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-sm font-bold text-purple-700">
                  {{ formatCurrency(item.avgPerTransaction) }}
                </div>
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                ...
                <span
                  v-else
                  class="px-3 py-2 text-xs text-gray-400 bg-gray-100 rounded-xl font-medium"
                  >-</span
                >
              </td>
      ...
      <div
        v-if="summaryData.length > 10"
        class="mt-6 flex items-center justify-between bg-gradient-to-r from-slate-50 to-slate-100 rounded-xl p-4 border border-slate-200"
      >
        <div class="text-sm font-medium text-slate-700">
          <span class="flex items-center">
            ...
            Menampilkan {{ Math.min(10, summaryData.length) }} dari
            {{ summaryData.length }} periode
          </span>
        </div>
        <div class="flex gap-2">
          <button
            class="px-4 py-2 text-sm font-medium bg-white text-slate-600 rounded-lg hover:bg-slate-50 border border-slate-200 hover:border-slate-300 transition-all duration-200 shadow-sm hover:shadow"
          >
            ← Sebelumnya
          </button>
          <button
    ...
    >
      <div class="flex items-center mb-6">
        <div
          class="p-3 bg-gradient-to-r from-purple-500 to-purple-600 rounded-xl shadow-lg"
        >
          <svg
            class="w-6 h-6 text-white"
            fill="none"
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <!-- Highest Day -->
        <div
          class="group bg-gradient-to-br from-green-50 to-emerald-50 rounded-2xl p-6 border border-green-200 hover:shadow-lg transition-all duration-300 transform hover:scale-105"
        >
          <div class="flex items-center">
            <div
              class="p-4 bg-gradient-to-r from-green-500 to-emerald-500 rounded-xl shadow-lg group-hover:shadow-xl transition-all duration-300"
            >
              <svg
                class="w-6 h-6 text-white"
                fill="currentColor"
            ...
            <div class="ml-4 flex-1">
              ...
              <p
                class="text-lg font-bold text-green-900 group-hover:text-green-800 transition-colors"
              >
                {{ bestDay.date }}
              </p>
              <p class="text-xs text-green-600 mt-1">
                {{ formatCurrency(bestDay.omzet) }}
              </p>
            </div>
          </div>
        </div>
        ...
        <!-- Growth Trend -->
        <div
          class="group bg-gradient-to-br from-blue-50 to-indigo-50 rounded-2xl p-6 border border-blue-200 hover:shadow-lg transition-all duration-300 transform hover:scale-105"
        >
          <div class="flex items-center">
            <div
              class="p-4 bg-gradient-to-r from-blue-500 to-indigo-500 rounded-xl shadow-lg group-hover:shadow-xl transition-all duration-300"
            >
              <svg
                class="w-6 h-6 text-white"
                fill="currentColor"
            ...
            <div class="ml-4 flex-1">
              ...
              <p
                class="text-lg font-bold text-blue-900 group-hover:text-blue-800 transition-colors"
              >
                {{ overallGrowth >= 0 ? "+" : ""
        ...
        <!-- Average per Period -->
        <div
          class="group bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-6 border border-purple-200 hover:shadow-lg transition-all duration-300 transform hover:scale-105"
        >
          <div class="flex items-center">
            <div
              class="p-4 bg-gradient-to-r from-purple-500 to-pink-500 rounded-xl shadow-lg group-hover:shadow-xl transition-all duration-300"
            >
              <svg
                class="w-6 h-6 text-white"
                fill="currentColor"
            ...
            <div class="ml-4 flex-1">
              ...
              <p
                class="text-lg font-bold text-purple-900 group-hover:text-purple-800 transition-colors"
              >
                {{ formatCurrency(averagePerPeriod) }}
              </p>
              <p class="text-xs text-purple-600 mt-1">
                {{ formatNumber(avgTransactionsPerPeriod) }} transaksi
              </p>
            </div>
          </div>
        </div>
...
const averagePerPeriod = computed(() => {
  ...
  return (
    chartData.value.reduce((sum, item) => sum + item.omzet, 0) /
...
const avgTransactionsPerPeriod = computed(() => {
  if (chartData.value.length === 0) return 0;
  return (
    chartData.value.reduce((sum, item) => sum + item.transaksi, 0) /
    chartData.value.length
  );
});
...
const fetchTrendData = async () => {
  ...
  try {
    ...
    let query = supabase
      .from("sbs.transaksi")
      .select("tanggal, total, total_item");
...
const groupTransactionsByPeriod = (transactions: any[]): ChartDataPoint[] => {
  ...
  return Array.from(groups.entries())
    .map(([period, periodTransactions]) => {
      const omzet = periodTransactions.reduce(
        (sum, t) => sum + Number(t.total),
        0
      );
      const transaksi = periodTransactions.length;
      const totalItems = periodTransactions.reduce(
...
const calculateMovingAveragesAndGrowth = (data: ChartDataPoint[]) => {
  ...
  data.forEach((item, index) => {
    ...
    if (showMovingAverage.value && index >= windowSize - 1) {
      ...
      item.movingAverage =
        window.reduce((sum, w) => sum + w.omzet, 0) / windowSize;
    }
    ...
    if (index > 0) {
      ...
      if (previous) {
        item.growth =
          previous.omzet > 0
            ? ((item.omzet - previous.omzet) / previous.omzet) * 100
            : 0;
      }
    }
  });
